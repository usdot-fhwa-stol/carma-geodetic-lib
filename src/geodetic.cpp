...
auto calculate_utm_zone(const Wgs84Coordinate & coordinate) -> UtmZone
{
  ...
  const auto number{
    static_cast<std::size_t>(
      (std::floor(carma_geodetic::remove_units(coordinate.longitude) + 180) /
       zone_width)) +
    1};
...
auto project_to_utm(const Wgs84Coordinate & coordinate) -> UtmCoordinate
{
  ...
  auto coord_wgs84 = proj_coord(
    carma_geodetic::remove_units(coordinate.latitude),
    carma_geodetic::remove_units(coordinate.longitude), 0, 0);
  ...
  -> units::angle::degree_t
{
  ...
  const auto factors = proj_factors(
    transform, proj_coord(
                 proj_torad(carma_geodetic::remove_units(position.longitude)),
                 proj_torad(carma_geodetic::remove_units(position.latitude)), 0, 0));
